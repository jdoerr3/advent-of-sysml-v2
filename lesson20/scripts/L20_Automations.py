...
class ReportGenerator:
    ...
    def generate_summary_table(data):
        ...
        table.append(f"| Number of bags | {data['number_of_bags']} bags |")
        table.append(f"| Total cargo weight | {data['cargo_weight']:.2f} kg |")
        table.append(f"| Sleigh total weight | {data['sleight_weight']:.2f} kg |")
        table.append(f"| Load per reindeer | {data['load_per_reindeer']:.2f} kg |")
        table.append(f"| Safety margin | {data['safety_margin']:.2f} kg |")
        table.append(f"| Flight mode | {data['flight_mode']} |")
        table.append(f"| Max allowed speed | {data['max_speed']:.2f} m/s |")
        table.append("\n---\n")
